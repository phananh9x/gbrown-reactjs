...
const FieldGroup = ({
  ...
  <div controlId={id} style={{ marginBottom: 10 }} className="app-from-group col-xs-12">
    ...
    <div className="col-xs-8">
      {(textArea
      && (<FormControl type={type} value={(valueDefault && valueDefault) || (value[id] && (type === 'date') && moment(value[id]).format('YYYY-MM-DD')) || (id === 'description-hangmuc' && value.description) || (value[id] && value[id]) || ''} componentClass="textarea" onChange={e => handleChange(id, e.target.value, thongTinHangMuc)} {...props} />))
